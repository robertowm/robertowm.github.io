...
import { style as containerStyle, HtmlBlock, TextBlock } from '../components/block';
    ...
    render() {
        ...
        return <div class="my-4">
            <Row style={{ fontSize: "14px" }}>
                <Col md="4">
                    <Img fluid={thumbnail.fluid} className="rounded img-thumbnail" alt={`Project ${name}`} />
                </Col>
                <Col md="8">
                    <h5>{name}</h5>
                    <a href={url}><h6>{company}</h6></a>
                    <p>
                        {summary}
                        &nbsp;
                        <Link to={path} style={{ color: "black", fontFamily: "monospace" }}>[>>]</Link>
                    </p>
                </Col>
            </Row>
        </div>;
